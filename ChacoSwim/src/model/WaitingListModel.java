...
//		String locationID = dbm.gotId("location", table.getValueAt(row, 4).toString());
	...
	public String registerNew(JTable table,int row){
		String sid = "",fn="",ln="";
		if(table.getValueAt(row, 1)!=null)
		fn = table.getValueAt(row, 1).toString();
		if(table.getValueAt(row, 2)!=null)
		ln = table.getValueAt(row, 2).toString();
		String level = table.getValueAt(row, 5).toString();
		
		String query = "INSERT INTO students (firstname,lastname,curlevel) VALUES ('"+fn+"','"+ln+"','"+level+"')";
		csm.executeWithoutRS(query);
		String query2 = "SELECT * FROM students order by sid DESC LIMIT 1";
		CachedRowSet crs = csm.excuteWithRS(query2);
		try {
			while( crs.next()){
				sid = crs.getString("sid");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
